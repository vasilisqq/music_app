...
class LessonCard(QFrame):
    ...
    def __init__(
        self, title="", progress=0.0, payload=None, status="available", parent=None
    ):
        ...
        self.title_label.setStyleSheet(
            f"font-size: 19px; font-weight: 800; color: {text_color}; background: transparent;"
        )
        ...
        if self.status == "locked":
            ...
            self.percent_label.setStyleSheet(
                "font-size: 17px; color: #28a745; background: transparent;"
            )
        else:
            self.percent_label = QLabel(f"{int(self._progress * 100)}%")
            self.percent_label.setStyleSheet(
                "font-size: 17px; font-weight: bold; color: #3f8bde; background: transparent;"
            )
    ...
    def paintEvent(self, event):
        ...
        if self.status == "locked":
            ...
            bg_color = (
                QColor(255, 255, 255) if self.underMouse() else QColor(252, 252, 252)
            )
