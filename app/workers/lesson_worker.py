...
class LessonWorker(BaseAPIWorker):
    ...
    lesson_error_sygnal = pyqtSignal(str) # Оставил с опечаткой как у тебя, чтобы не сломать UI
    ...
    def create_lesson(self, lesson_data: LessonCreate) -> None:
        self._make_request(
            ...
            data=lesson_data,
            success_callback=lambda d: self.lesson_created_sygnal.emit(LessonResponse.model_validate(d)),
    ...
    def get_lesson_by_id(self, lesson_id: int) -> None:
        self._make_request(
            ...
            success_callback=lambda d: self.lesson_get_signal.emit(LessonResponse.model_validate(d)),
            error_callback=self.lesson_error_sygnal.emit
        )
    ...
    def update_lesson(self, lesson_id: int, lesson_data: LessonUpdate) -> None:
        self._make_request(
            ...
            endpoint=f"/lesson/{lesson_id}",
            data=lesson_data,
            success_callback=lambda d: self.lesson_updated_signal.emit(LessonResponse.model_validate(d)),
            error_callback=self.lesson_error_sygnal.emit
        )
